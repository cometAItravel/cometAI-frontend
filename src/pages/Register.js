...
export default function Register() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND}/register`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({...form, ref}) });
      ...
      <div style={{position:"relative",zIndex:2,width:"100%",maxWidth:420,animation:"fadeUp 0.6s both"}}>
        <div style={{background:"rgba(255,255,255,0.88)",backdropFilter:"blur(24px)",borderRadius:24,padding:"clamp(28px,5vw,48px) clamp(20px,5vw,40px)",boxShadow:"0 24px 80px rgba(0,0,0,0.08)",border:"1px solid rgba(201,168,76,0.15)"}}>
          <div style={{textAlign:"center",marginBottom:36}}>
            ...
            <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:700,fontSize:30,color:"#1a1410",marginBottom:6}}>Create account</h1>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,color:"#888"}}>Join Alvryn — it's free forever</p>
          </div>

          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            {[{key:"name",label:"Full Name",type:"text",ph:"Your full name"},{key:"email",label:"Email",type:"email",ph:"you@example.com"},{key:"password",label:"Password",type:"password",ph:"Min. 6 characters"}].map(f=>(
              <div key={f.key}>
            ...
            <button onClick={handleSubmit} disabled={loading}
              style={{padding:"14px",borderRadius:13,fontSize:15,fontWeight:700,fontFamily:"'Cormorant Garamond',serif",letterSpacing:"0.08em",color:"#1a1410",border:"none",cursor:loading?"not-allowed":"pointer",background:loading?"#e5e0d8":GRAD,backgroundSize:"200% 200%",animation:loading?"none":"gradShift 4s ease infinite",boxShadow:loading?"none":"0 8px 32px rgba(201,168,76,0.38)",marginTop:6,opacity:loading?0.7:1,transition:"transform 0.2s,opacity 0.2s"}}
              onMouseEnter={e=>!loading&&(e.currentTarget.style.transform="translateY(-2px)")}
              onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
              {loading?"Creating account…":"Create Free Account ✈"}
            </button>

            <p style={{textAlign:"center",fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#ccc",lineHeight:1.6}}>
              By signing up you agree to our{" "}
              <span style={{color:GOLD_DARK,cursor:"pointer",fontStyle:"italic"}}>Terms</span> &amp;{" "}
              <span style={{color:GOLD_DARK,cursor:"pointer",fontStyle:"italic"}}>Privacy Policy</span>
            </p>
          </div>
          ...
          <p style={{textAlign:"center",fontFamily:"'DM Sans',sans-serif",fontSize:14,color:"#888",marginTop:16}}>
            ...
            <span onClick={()=>navigate("/login")} style={{color:GOLD_DARK,cursor:"pointer",fontWeight:700,fontStyle:"italic",textDecoration:"underline",textUnderlineOffset:2}}>Sign in →</span>
          </p>
        </div>
      </div>
    </div>
