...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes twinkle { 0%,100%{opacity:var(--min-op,.2);}50%{opacity:1;} }
  ...
  @keyframes shoot { 0%{transform:translate(0,0) rotate(var(--angle));opacity:1;}100%{transform:translate(var(--tx),var(--ty)) rotate(var(--angle));opacity:0;} }
  ...
  .logo { display:flex; align-items:center; gap:10px; margin-bottom:24px; animation:fadeUp 0.6s ease both; }
  ...
  .badge { display:inline-flex; align-items:center; gap:8px; background:rgba(99,102,241,0.1); border:1px solid rgba(129,140,248,0.25); color:#a5b4fc; padding:8px 18px; border-radius:20px; font-size:11px; letter-spacing:2px; text-transform:uppercase; margin-bottom:18px; animation:fadeUp 0.6s ease 0.1s both; }
  ...
  .subtitle { font-size:15px; color:rgba(180,190,255,0.55); max-width:460px; line-height:1.7; margin-bottom:24px; animation:fadeUp 0.6s ease 0.3s both; }

  .ref-info { width:100%; max-width:500px; background:rgba(99,102,241,0.08); border:1px solid rgba(129,140,248,0.2); border-radius:16px; padding:20px 22px; margin-bottom:20px; text-align:left; animation:fadeUp 0.6s ease 0.35s both; }
  .ref-info-title { font-family:'Orbitron',sans-serif; font-size:11px; font-weight:600; color:#a5b4fc; letter-spacing:1px; margin-bottom:14px; text-align:center; }
  .step { display:flex; align-items:flex-start; gap:12px; margin-bottom:11px; }
  .step:last-child { margin-bottom:0; }
  .step-icon { font-size:17px; flex-shrink:0; }
  .step-txt { font-size:13px; color:rgba(165,180,252,0.65); line-height:1.5; }
  .step-txt strong { color:#e0e7ff; }
  .ref-info-note { font-size:11px; color:rgba(165,180,252,0.28); margin-top:12px; padding-top:12px; border-top:1px solid rgba(255,255,255,0.06); text-align:center; }
  ...
  .av { width:28px; height:28px; border-radius:50%; border:2px solid #01020a; background:linear-gradient(135deg,#6366f1,#8b5cf6); display:flex; align-items:center; justify-content:center; font-size:11px; margin-left:-7px; }
  .av:first-child { margin-left:0; }
  .counter-txt { font-size:13px; color:rgba(165,180,252,0.6); }
  .counter-num { color:#a5b4fc; font-weight:600; }

  .form { width:100%; max-width:500px; animation:fadeUp 0.6s ease 0.45s both; }
  ...
  .hint { font-size:11px; color:rgba(165,180,252,0.28); }
  .err { background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.2); color:#f87171; padding:10px 16px; border-radius:10px; font-size:13px; margin-bottom:10px; }
  ...
  .s-icon { font-size:36px; margin-bottom:8px; }
  .s-title { font-family:'Orbitron',sans-serif; font-size:16px; font-weight:700; color:#6ee7b7; margin-bottom:6px; }
  .s-sub { font-size:13px; color:rgba(110,231,183,0.6); }

  .ref-card { background:rgba(99,102,241,0.08); border:1px solid rgba(129,140,248,0.2); border-radius:16px; padding:20px 22px; margin-bottom:12px; }
  ...
  .r-amt { font-family:'Orbitron',sans-serif; font-size:20px; font-weight:800; color:#6ee7b7; }
  .r-lbl { font-size:11px; color:rgba(165,180,252,0.4); margin-top:4px; line-height:1.4; }
  ...
  .stat-v { font-family:'Orbitron',sans-serif; font-size:22px; font-weight:800; color:#e0e7ff; }
  .stat-l { font-size:11px; color:rgba(165,180,252,0.4); margin-top:3px; }
  ...
  .link-txt { flex:1; font-size:11px; color:#a5b4fc; font-family:monospace; text-align:left; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  ...
  .share-lbl { font-size:11px; color:rgba(165,180,252,0.35); letter-spacing:1px; text-transform:uppercase; margin-bottom:10px; }
  ...
  .lb-title { font-family:'Orbitron',sans-serif; font-size:11px; color:rgba(165,180,252,0.35); letter-spacing:2px; text-transform:uppercase; margin-bottom:6px; }
  ...
  .foot { margin-top:32px; font-size:12px; color:rgba(165,180,252,0.2); animation:fadeUp 0.6s ease 0.7s both; }
  ...
  @media(max-width:480px){
    ...
    .sbtn { padding:9px 12px; font-size:12px; }
  }
`;
...
function ShootingStars() {
  ...
  useEffect(()=>{
    ...
    const launch=()=>{
      ...
      const s={id:id++,x,y,tx:Math.cos(rad)*dist,ty:Math.sin(rad)*dist,angle,dur:700+Math.random()*700};
      setStars(p=>[...p,s]);
      setTimeout(()=>setStars(p=>p.filter(x=>x.id!==s.id)),s.dur+100);
...
function Waitlist() {
  ...
  // get incoming referral code from URL (friend's code) — read once
  const incomingRef = (() => {
  ...
  useEffect(() => {
    // always fetch count and leaderboard
    ...
    // NOTE: we do NOT restore session from localStorage anymore
    // User always sees the join form — they must enter email to see their dashboard
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (err.response?.status === 409) {
        // already on waitlist — get their existing ref code
        const code = err.response.data.refCode;
        if (code) {
          setMyRefCode(code);
          // fetch their referral count
          axios.get(`${API}/waitlist/my-refs/${code}`).then(r=>setMyRefs(r.data.count||0)).catch(()=>{});
          setStatus("success");
        } else {
  ...
  // referral link — ONLY ref code, no email — so friends see join form
  ...
  const shareIG = () => {
    navigator.clipboard.writeText(`✈️ Join CometAI waitlist and get ₹100 off your first flight!\n👉 ${refLink}`);
  ...
  return (
    <>
      ...
      <div className="page">
        ...
        <div className="badge"><div className="badge-dot"/>Launching Soon</div>

        <h1 className="title">The Future of<br/>Travel Booking</h1>
        <p className="subtitle">Book flights using AI or WhatsApp. Zero booking fees. India's smartest travel platform is almost here!</p>

        {status !== "success" ? (
          <>
            {/* REFERRAL INFO — always shown before joining */}
            <div className="ref-info">
              <div className="ref-info-title">🎁 Join & Earn Launch Rewards</div>
              <div className="step">
                <span className="step-icon">1️⃣</span>
                <div className="step-txt">Enter your email and join — get your <strong>unique referral link</strong> instantly in your email</div>
              </div>
              <div className="step">
                <span className="step-icon">2️⃣</span>
                <div className="step-txt">Share your link on <strong>WhatsApp, Instagram or Twitter</strong> with friends and family</div>
              </div>
              <div className="step">
                <span className="step-icon">3️⃣</span>
                <div className="step-txt">When your friend books a flight above <strong>₹5,000</strong> — <strong>you get ₹150 off</strong> your next booking and <strong>your friend gets ₹100 off</strong> their first booking. <strong>No limit</strong> on referrals!</div>
              </div>
              <div className="ref-info-note">📧 Already joined? Enter your email again to see your referral dashboard</div>
            </div>
            ...
            <div className="counter">
              ...
              <div className="counter-txt"><span className="counter-num">{count}+</span> people already joined</div>
            </div>
          ...
          <div className="success-wrap">
            <div className="success-box">
              ...
              <div className="s-sub">Your referral link was sent to <strong style={{color:"#a5b4fc"}}>{email}</strong>. Share it to earn ₹150 per referral!</div>
            </div>

            <div className="ref-card">
              <div className="ref-card-title">✦ Your Referral Rewards</div>
              <div className="ref-card-desc">
                Share your link with everyone — <strong>every person</strong> who books a flight above ₹5,000 using your link earns you ₹150 off. No limit on referrals!
              </div>

              <div className="rewards">
                <div className="reward">
                  <div className="r-amt">₹150</div>
                  <div className="r-lbl">You get per referral (no limit!)</div>
                </div>
                <div className="reward">
                  <div className="r-amt">₹100</div>
                  <div className="r-lbl">Friend gets on first booking</div>
                </div>
              </div>

              <div className="stats">
                <div className="stat">
                  <div className="stat-v">{myRefs}</div>
                  <div className="stat-l">Friends referred</div>
                </div>
                <div className="stat">
                  <div className="stat-v">₹{(myRefs * 150).toLocaleString()}</div>
                  <div className="stat-l">Bonus earned</div>
                </div>
              </div>

              <div className="link-row">
                <div className="link-txt">{refLink}</div>
                <button className={`btn-copy ${copied?"done":""}`} onClick={copyRef}>{copied?"✓ Copied!":"Copy"}</button>
              </div>

              <div className="share-lbl">Share now & start earning</div>
              <div className="share-btns">
                ...
                <button className="sbtn sbtn-cp" onClick={copyLink}>{linkCopied?"✓ Copied!":"🔗 Copy link"}</button>
              </div>
            </div>
            ...
            <div style={{marginTop:"12px"}}>
              <button
                style={{background:"transparent",border:"1px solid rgba(255,255,255,0.08)",color:"rgba(165,180,252,0.4)",padding:"7px 16px",borderRadius:"8px",fontFamily:"'DM Sans',sans-serif",fontSize:"12px",cursor:"pointer"}}
                onClick={()=>{setStatus("idle");setEmail("");setMyRefCode("");}}
        ...
        {leaderboard.length > 0 && (
          <div className="lb-wrap">
            ...
            <div className="lb-note">Everyone earns ₹150 per referral — leaderboard is just for fun!</div>
