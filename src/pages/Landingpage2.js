...
function AlvrynMark({size=40,glow=false}){
  const s=size,sw=Math.max(1,s*0.048);
  return(
    <svg width={s} height={s*0.88} viewBox="0 0 100 88" fill="none"
      ...
      <line x1="50" y1="3" x2="3" y2="85" stroke={"url(#ig"+s+")"} strokeWidth={sw} strokeLinecap="round"/>
      <line x1="50" y1="3" x2="97" y2="85" stroke={"url(#ig"+s+")"} strokeWidth={sw} strokeLinecap="round"/>
    </svg>
  );
}
...
function MiniAlviChat({visible,onTry}){
  ...
  return(
    <div style={{opacity:visible?1:0,transform:visible?"translateY(0)":"translateY(24px)",transition:"opacity 0.9s 0.8s ease,transform 0.9s 0.8s ease",animation:visible?"cardFloat 5s ease-in-out 2s infinite":"none"}}>
      <div style={{background:"rgba(10,6,4,0.72)",backdropFilter:"blur(28px)",border:"1px solid rgba(201,168,76,0.18)",borderRadius:22,overflow:"hidden",boxShadow:"0 32px 80px rgba(0,0,0,0.45)"}}>
        ...
        <div style={{padding:"10px 14px",borderTop:"1px solid rgba(255,255,255,0.04)",display:"flex",gap:8,alignItems:"center"}}>
          ...
          <button onClick={onTry} style={{width:34,height:34,borderRadius:"50%",background:"linear-gradient(135deg,"+GD+","+G+")",border:"none",cursor:"pointer",fontSize:16,color:"#030303",display:"flex",alignItems:"center",justifyContent:"center"}}>↑</button>
        </div>
      </div>
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
      <div style={{background:"#f5f0eb",borderRadius:20,padding:"40px",maxWidth:560,width:"100%",maxHeight:"80vh",overflowY:"auto",position:"relative"}} onClick={e=>e.stopPropagation()}>
        <button onClick={onClose} style={{position:"absolute",top:16,right:16,background:"rgba(0,0,0,0.08)",border:"none",borderRadius:"50%",width:32,height:32,cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",color:"#555"}}>×</button>
...
export default function LandingPage2(){
  ...
  const [alvrynVis,setAlvrynVis]=useState(false);
  ...
  useEffect(()=>{
    ...
    T(()=>setAlvrynVis(true),300);
    T(()=>setTagVis(true),1100);
    T(()=>{setLifting(true);setNavVis(true);},2600);
  ...
  const PRIVACY="Alvryn collects only the information necessary to provide travel planning services. We do not sell your data to third parties. You may request deletion of your data at any time by contacting us at privacy@alvryn.in. We use industry-standard encryption for all data in transit and at rest.";
  const TERMS="By using Alvryn and ALVI, you agree to use our services for lawful travel planning purposes only. We are not responsible for bookings made through third-party partners (Aviasales, RedBus, Booking.com, IRCTC). Prices shown are indicative and subject to availability. Disputes shall be governed under Indian law.";
  const ABOUT="Alvryn is India's first AI-powered travel planning platform, built in Bangalore. Our AI assistant ALVI understands your trip requirements in plain language and plans complete journeys in seconds. We earn a small commission from booking partners when you book through our links, at no extra cost to you.";

  const DESTS=[
    {n:"Goa",t:"Beaches",b:"5k-15k",i:"photo-1512343879784-a960bf40e7f2"},
    {n:"Dubai",t:"Global",b:"35k-70k",i:"photo-1512453979798-5ea266f8880c"},
    {n:"Manali",t:"Hills",b:"8k-18k",i:"photo-1626621341517-bbf3d9990a23"},
    {n:"Singapore",t:"City",b:"40k-80k",i:"photo-1525625293386-3f8f99389edd"},
    {n:"Bali",t:"Tropical",b:"25k-55k",i:"photo-1537996194471-e657df975ab4"},
    {n:"Switzerland",t:"Alps",b:"80k-1.5L",i:"photo-1530122037265-a5f1f91d3b99"},
    {n:"Maldives",t:"Luxury",b:"40k-1L",i:"photo-1514282401047-d79a71a590e8"},
    {n:"Japan",t:"Culture",b:"60k-1.2L",i:"photo-1528360983277-13d401cdc186"},
    {n:"Kerala",t:"Nature",b:"10k-25k",i:"photo-1602216056096-3b40cc0c9944"},
    {n:"Ladakh",t:"Adventure",b:"18k-35k",i:"photo-1544735716-392fe2489ffa"},
    {n:"Paris",t:"Romance",b:"80k-1.5L",i:"photo-1502602898657-3e91760cbb34"},
    {n:"Bangkok",t:"Culture",b:"20k-45k",i:"photo-1563492065599-3520f775eeed"},
  ];
  ...
  const PLANS=[
    ...
    {name:"Navigator",badge:"PRO",active:false,desc:"For the serious traveller",features:["Unlimited trip plans","Advanced AI planning","Budget optimizer","Multi-city planning","Priority responses","Trip history"]},
    {name:"Voyager",badge:"PREMIUM",active:false,desc:"The ultimate companion",features:["Everything in Navigator","Group travel planner","Scam protection alerts","Women traveller mode","Emergency companion","Real-time weather"]},
  ];

  const CONSTRAINTS=[
    {icon:"people",t:"Group of 6",d:"Per person and total budget breakdown"},
    {icon:"veg",t:"2 Vegetarians",d:"Specific restaurant picks in South Goa"},
    {icon:"time",t:"Late arrival",d:"Separate travel plan auto-created"},
    {icon:"beach",t:"Beaches only",d:"South Goa recommended over North Goa"},
    {icon:"budget",t:"15k budget",d:"Confirmed: fits at 11500-14500 per person"},
    {icon:"shield",t:"Safety insights",d:"Goa safety tips automatically included"},
  ];

  const ICONMAP={people:"👥",veg:"🥗",time:"🕐",beach:"🏖️",budget:"💰",shield:"🛡️"};
  ...
  return(
    <>
      ...
      <div style={{position:"fixed",inset:0,zIndex:100,background:"#ffffff",transform:lifting?"translateY(-100%)":"translateY(0%)",transition:lifting?"transform 1.8s cubic-bezier(0.76,0,0.24,1)":"none",pointerEvents:introDone?"none":"all",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:300,fontSize:"clamp(56px,10vw,128px)",letterSpacing:"0.4em",color:"#0a0a0a",textAlign:"center",lineHeight:1,paddingLeft:"0.4em",opacity:alvrynVis?1:0,transform:alvrynVis?"scale(1)":"scale(0.92)",transition:"opacity 0.9s cubic-bezier(0.22,1,0.36,1), transform 0.9s cubic-bezier(0.22,1,0.36,1)"}}>ALVRYN</div>
        <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:300,fontSize:"clamp(10px,1.2vw,13px)",color:"rgba(0,0,0,0.28)",letterSpacing:"0.32em",textAlign:"center",marginTop:20,opacity:tagVis?1:0,transition:"opacity 0.8s ease"}}>TRAVEL BEYOND BOUNDARIES</div>
      </div>

      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:200,height:64,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 5%",background:navScrolled?"rgba(8,5,4,0.92)":"transparent",backdropFilter:navScrolled?"blur(24px)":"none",borderBottom:navScrolled?"1px solid rgba(255,255,255,0.04)":"none",transition:"all 0.5s ease",opacity:navVis?1:0,transform:navVis?"translateY(0)":"translateY(-10px)",pointerEvents:navVis?"all":"none"}}>
      ...
      <section style={{minHeight:"100vh",position:"relative",overflow:"hidden",background:"linear-gradient(160deg,#0a0604 0%,#160a04 30%,#2d1408 60%,#8B3A0F 80%,rgba(201,168,76,0.25) 100%)",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{position:"relative",zIndex:5,flex:1,display:"grid",gridTemplateColumns:"1fr 1fr",gap:40,alignItems:"center",padding:"120px 6% 60px"}} className="g2">
          <div>
            <div style={{opacity:heroVis?1:0,transform:heroVis?"translateY(0)":"translateY(20px)",transition:"opacity 0.8s 0.1s ease,transform 0.8s 0.1s ease",marginBottom:28}}>
              ...
              <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:200,fontSize:"clamp(48px,6.5vw,100px)",color:"#fff",lineHeight:0.95,letterSpacing:"-0.02em"}}>Turn dreams<br/><span className="g-text">into journeys.</span></h1>
            </div>
            <div style={{opacity:heroVis?1:0,transform:heroVis?"translateY(0)":"translateY(16px)",transition:"opacity 0.9s 0.35s ease,transform 0.9s 0.35s ease",marginBottom:36}}>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"clamp(15px,1.6vw,18px)",color:"rgba(255,255,255,0.45)",lineHeight:1.7,maxWidth:460}}>
                <TypeWriter phrases={["Flights. Hotels. Itineraries. In one message.","Plan Goa for 6 friends in 30 seconds.","Your complete trip. Planned instantly.","Budget travel or luxury — ALVI knows both.","WhatsApp, web or app. ALVI is everywhere."]}/>
              </p>
            </div>
        ...
        <div style={{position:"relative",zIndex:5,display:"flex",justifyContent:"space-between",padding:"0 4% 1.5%",opacity:heroVis?1:0,transition:"opacity 1.2s 1s ease",pointerEvents:"none"}}>
          {"DREAM".split("").map((l,i)=>(<span key={l+i} style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:200,fontSize:"clamp(44px,7vw,110px)",color:"rgba(255,255,255,0.065)",lineHeight:1}}>{l}</span>))}
        </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"center"}} className="g2">
            <div>
              <Reveal>
                ...
                <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:200,fontSize:"clamp(36px,4.5vw,64px)",color:"#0a0a0a",lineHeight:1.05,marginBottom:28}}>Most people never go<br/>beyond what they know.</h2>
                <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:16,color:"rgba(0,0,0,0.48)",lineHeight:1.85,maxWidth:420,marginBottom:36}}>Every great journey starts as an impossible idea. The world is full of people who say "one day." ALVI exists for the ones who say "this year."</p>
              </Reveal>
              <Reveal delay={150}>
                <div style={{display:"flex",flexDirection:"column",gap:0}}>
                  {[{n:"01",t:"You imagine the trip",d:"Goa. Bali. Switzerland. The northern lights. Anywhere at all."},{n:"02",t:"ALVI makes it real",d:"Flights, hotels, cabs, itinerary — one message. Seconds."},{n:"03",t:"You live the dream",d:"No tab overload. No spreadsheets. Just you and the journey."}].map((f,i)=>(
              ...
              <div style={{position:"relative"}}>
                <div style={{background:"#0a0604",borderRadius:24,padding:"48px 40px",position:"relative",overflow:"hidden"}}>
                  <div style={{position:"relative",zIndex:2}}>
                    <div style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:200,fontSize:"clamp(22px,2.8vw,34px)",color:"#fff",lineHeight:1.38,marginBottom:28,fontStyle:"italic"}}>"The world is bigger than your comfort zone. You were made to explore it."</div>
      ...
      <section id="s-alvi" style={{background:"#100818",padding:"120px 6%",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{maxWidth:1100,margin:"0 auto",position:"relative",zIndex:2}}>
          <Reveal style={{marginBottom:64,textAlign:"center"}}>
            ...
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:200,fontSize:"clamp(32px,4.5vw,64px)",color:"#fff",lineHeight:1.1}}>One message.<br/><span style={{color:"#b085ff"}}>Your entire journey planned.</span></h2>
          </Reveal>
            ...
            <Reveal direction="left">
              <div style={{borderRadius:24,overflow:"hidden",border:"1px solid rgba(255,255,255,0.06)",background:"rgba(0,0,0,0.48)",backdropFilter:"blur(20px)"}}>
                ...
                <div style={{padding:"20px",display:"flex",flexDirection:"column",gap:12}}>
                  {[{role:"user",text:"6 friends Bangalore to Goa in August. Budget 15k per person. Two vegetarians. One arrives a day late. Prefer beaches."},{role:"ai",text:"Perfect! Planning this right now for all 6 of you."},{role:"ai",text:"Flights BLR to GOI: 3500-4500 per person return. Book 4-6 weeks early. For 6 total: 21000-27000."},{role:"ai",text:"South Goa near Palolem and Agonda. Budget 800-1800 per room. Veg restaurants picked. Late arrival has separate plan.",card:true}].map((m,i)=>(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:36}}>
                {CONSTRAINTS.map((f,i)=>(
                  <div key={i} style={{display:"flex",gap:12,padding:"12px 14px",borderRadius:12,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.05)",transition:"all 0.3s ease",cursor:"default"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(180,140,255,0.08)";e.currentTarget.style.borderColor="rgba(180,140,255,0.2)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.03)";e.currentTarget.style.borderColor="rgba(255,255,255,0.05)";}}>
                    <span style={{fontSize:18}}>{ICONMAP[f.icon]}</span>
            ...
            {DESTS.map((d,i)=>(
              <Reveal key={d.n} delay={i*38}>
                <div onClick={()=>navigate(localStorage.getItem("token")?"/ai?dest="+d.n:"/register")} className="lift" style={{borderRadius:18,overflow:"hidden",cursor:"pointer",position:"relative",aspectRatio:"2/3"}}>
                  <img src={"https://images.unsplash.com/"+d.i+"?auto=format&fit=crop&w=400&h=600&q=70"} alt={d.n} loading="lazy" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}} onError={e=>{e.target.style.display="none";e.target.parentElement.style.background="rgba(201,168,76,0.1)";}}/>
                ...
                <div style={{borderRadius:24,overflow:"hidden",height:"100%",position:"relative",background:p.active?"#0a0604":"rgba(0,0,0,0.04)",border:p.active?"none":"1px solid rgba(0,0,0,0.08)",boxShadow:p.active?"0 40px 100px rgba(10,6,4,0.22)":"none"}}>
                  ...
                  <div style={{padding:"36px 28px"}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:28}}>
                      {p.features.map(f=>(<div key={f} style={{display:"flex",gap:10,alignItems:"center"}}><span style={{color:p.active?G:GD,fontSize:14,flexShrink:0}}>&#x2726;</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:p.active?"rgba(255,255,255,0.7)":"rgba(0,0,0,0.6)"}}>{f}</span></div>))}
                    </div>
                    {p.active?<button onClick={goApp} className="btn-primary" style={{width:"100%",justifyContent:"center"}}>{p.cta}</button>:<div style={{padding:"12px",borderRadius:12,border:"1px solid rgba(0,0,0,0.1)",textAlign:"center"}}><div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"rgba(0,0,0,0.28)",letterSpacing:"0.1em"}}>Intelligent Travel Upgrades Are On The Way</div></div>}
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      <section style={{minHeight:"80vh",background:"#020202",display:"flex",alignItems:"center",justifyContent:"center",padding:"120px 6%",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:2,textAlign:"center",maxWidth:760}}>
          <Reveal>
            ...
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:200,fontSize:"clamp(44px,7.5vw,98px)",color:"#fff",lineHeight:0.95,letterSpacing:"-0.02em",marginBottom:24}}>Your next trip is<br/><span className="g-text">one message away.</span></h2>
      ...
      <footer style={{background:"#010101",borderTop:"1px solid rgba(255,255,255,0.04)",padding:"44px 6%"}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:20}}>
          ...
          <div style={{display:"flex",gap:20}}>
            <span onClick={()=>setModal({title:"About Alvryn",content:ABOUT})} style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"rgba(255,255,255,0.22)",cursor:"pointer"}} onMouseEnter={e=>e.target.style.color="rgba(255,255,255,0.65)"} onMouseLeave={e=>e.target.style.color="rgba(255,255,255,0.22)"}>About</span>
            <span onClick={()=>setModal({title:"Privacy Policy",content:PRIVACY})} style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"rgba(255,255,255,0.22)",cursor:"pointer"}} onMouseEnter={e=>e.target.style.color="rgba(255,255,255,0.65)"} onMouseLeave={e=>e.target.style.color="rgba(255,255,255,0.22)"}>Privacy</span>
            <span onClick={()=>setModal({title:"Terms of Use",content:TERMS})} style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"rgba(255,255,255,0.22)",cursor:"pointer"}} onMouseEnter={e=>e.target.style.color="rgba(255,255,255,0.65)"} onMouseLeave={e=>e.target.style.color="rgba(255,255,255,0.22)"}>Terms</span>
            <a href="mailto:hello@alvryn.in" style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"rgba(255,255,255,0.22)",textDecoration:"none"}} onMouseEnter={e=>e.target.style.color="rgba(255,255,255,0.65)"} onMouseLeave={e=>e.target.style.color="rgba(255,255,255,0.22)"}>Contact</a>
          </div>
        </div>
      </footer>
    </>
