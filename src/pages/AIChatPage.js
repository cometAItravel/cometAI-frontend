...
@keyframes thinkPulse{0%,100%{opacity:0.4;transform:scale(0.95);}50%{opacity:1;transform:scale(1.05);}}
@keyframes thinkDot{0%,80%,100%{transform:scale(0);}40%{transform:scale(1);}}
@keyframes emojiSpin{0%{opacity:0;transform:scale(0.5) rotate(-10deg);}50%{opacity:1;transform:scale(1.2) rotate(5deg);}100%{opacity:0;transform:scale(0.5) rotate(10deg);}}
.think-dot{width:6px;height:6px;border-radius:50%;background:#c9a84c;display:inline-block;animation:thinkDot 1.2s infinite ease-in-out;}
...
@media(max-width:768px){
  ...
  .sidebar.open{
    width:min(85vw,280px)!important;min-width:0!important;
...
.overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:390;}
...
// ── Typewriter hook ───────────────────────────────────────────────────────────
function useTypewriter(text, speed=18, skip=false){
    ...
    <div className="travel-card" onClick={()=>window.open(f.link,"_blank","noopener")}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            {f.insight&&<div style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginTop:2}}>💡 {f.insight}</div>}
          </div>
        </div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        <div style={{textAlign:"center",minWidth:60}}>
          ...
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"rgba(240,230,208,0.55)",marginTop:3}}>{f.fromCode}</div>
        </div>
        <div style={{flex:1,textAlign:"center",padding:"0 14px"}}>
          <div style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginBottom:4}}>{f.duration||"Direct"}</div>
        ...
        <div style={{textAlign:"center",minWidth:60}}>
          ...
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"rgba(240,230,208,0.55)",marginTop:3}}>{f.toCode}</div>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
        paddingTop:12,borderTop:`1px solid rgba(201,168,76,0.1)`}}>
        <div>
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"rgba(240,230,208,0.55)",marginBottom:3,letterSpacing:"0.1em"}}>APPROX FROM</div>
          ...
          {f.price&&<div style={{fontSize:10,color:"rgba(240,230,208,0.55)",marginTop:2}}>–₹{Math.round(f.price*1.22).toLocaleString()} · may vary</div>}
        </div>
    ...
    <div className="travel-card" onClick={()=>window.open(b.link,"_blank","noopener")}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginTop:2}}>{b.type}</div>
          </div>
        </div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginTop:2}}>{b.from}</div>
        </div>
        <div style={{flex:1,textAlign:"center",padding:"0 12px"}}>
          <div style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginBottom:4}}>{b.duration||"Direct"}</div>
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginTop:2}}>{b.to}</div>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
  ...
  return(
    <div className="travel-card" onClick={()=>window.open(h.link,"_blank","noopener")}
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:1,marginTop:3,alignItems:"center"}}>
              ...
              <span style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginLeft:5}}>& above</span>
            </div>
          </div>
        </div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
        ...
        <div>
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"rgba(240,230,208,0.55)",marginBottom:3,letterSpacing:"0.1em"}}>APPROX PER NIGHT</div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:"rgba(240,230,208,0.55)",marginTop:2}}>Indian Railways · IRCTC</div>
          </div>
        </div>
      ...
      <div className="sb-item" onClick={()=>{if(!menuOpen&&!renaming)onLoad();}}
        ...
        <div style={{flex:1,minWidth:0}}>
          {renaming?(
            ...
            <div style={{fontSize:13,color:"#f0e0c0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontFamily:"'DM Sans',sans-serif",fontWeight:500}}>{chat.title}</div>
          )}
          <div style={{fontSize:10,color:"#c9a060",marginTop:2}}>{new Date(chat.time).toLocaleDateString("en-IN",{day:"numeric",month:"short"})}</div>
        </div>
        {/* 3-dot button */}
...
const C_STATIC = {
  ...
  // Sidebar is always dark — use light text colors
  textPri:"#f0e6d0",
  textSec:"rgba(240,230,208,0.75)",
...
export default function AIChatPage(){
  ...
  // Cycle travel emojis while thinking
  useEffect(()=>{
    if(!thinking) return;
    const emojis=["✈️","🚌","🏨","🚂","🗺️","🌍","🏖️","🛫"];
    let i=0;
    const t=setInterval(()=>{ i=(i+1)%emojis.length; setThinkEmoji(emojis[i]); },350);
    return()=>clearInterval(t);
  },[thinking]);
    ...
    try{
      ...
      if(data.text){
        ...
        for(let i=1; i<=fullText.length; i++){
          ...
          setMessages(prev=>prev.map(m=>m.id===aMsg.id?{...m,text:fullText.slice(0,i),_typing:i<fullText.length}:m));
          if(i%30===0) messagesEndRef.current?.scrollIntoView({behavior:"smooth"});
        }
        setMessages(prev=>prev.map(m=>m.id===aMsg.id?{...m,_typing:false}:m));
      }
              ...
              {chats.length>0&&(
                <>
                  <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,
                    color:"#c9a060",letterSpacing:"0.15em",marginBottom:8,paddingLeft:8}}>
                    RECENT
                  </div>
            ...
            <div style={{borderTop:`1px solid ${C.sbBorder}`,paddingTop:12}}>
              <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:10,padding:"0 4px"}}>
                ...
                <div style={{overflow:"hidden",flex:1}}>
                  <div style={{fontSize:13,color:"#f0e6d0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:500}}>{user.name||"Traveller"}</div>
                  <div style={{fontSize:10,color:"rgba(245,234,213,0.55)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user.email||""}</div>
                </div>
              </div>
                ...
                {[["🔍","Search","/search"],["👤","Profile","/profile"],["🏠","Home","/"]].map(([icon,label,path])=>(
                  <button key={path} className="sb-btn" onClick={()=>navigate(path)}
                    style={{padding:"7px 4px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",
                      background:"rgba(255,255,255,0.04)",color:"rgba(245,234,213,0.5)",
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button onClick={()=>setSbOpen(s=>!s)}
              style={{width:36,height:36,borderRadius:8,background:"rgba(255,255,255,0.04)",
                ...
                display:"flex",alignItems:"center",justifyContent:"center",
                color:"rgba(245,234,213,0.7)",transition:"all 0.15s",
                WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}
              onMouseEnter={e=>{e.currentTarget.style.background="rgba(201,168,76,0.08)";e.currentTarget.style.color=C.gold;}}
              onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.04)";e.currentTarget.style.color="rgba(245,234,213,0.7)";}}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/>
              </svg>
              <span className="ham-label">{sbOpen?"CLOSE":"HISTORY"}</span>
            </button>
        ...
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"clamp(10px,2vw,20px) clamp(8px,2vw,16px)",background:CT.msgBg||"#f0e8d4",minHeight:0,transition:"background 0.5s ease"}}>
          <div style={{maxWidth:740,margin:"0 auto"}}>
            ...
            {thinking&&(
              <div style={{display:"flex",alignItems:"flex-end",gap:10,marginBottom:16,animation:"fadeUp 0.3s both"}}>
                <div style={{width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.gold},${C.gold}88)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:16,animation:"thinkPulse 1s ease-in-out infinite"}}>
                  {thinkEmoji}
                </div>
                <div style={{padding:"12px 18px",borderRadius:"18px 18px 18px 4px",background:"rgba(255,255,255,0.9)",boxShadow:"0 2px 12px rgba(0,0,0,0.08)",border:"1px solid rgba(201,168,76,0.15)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:4}}>
                    <span style={{fontSize:11,color:"#8B6914",fontFamily:"'Space Mono',monospace",fontWeight:600,letterSpacing:"0.1em"}}>ALVRYN AI</span>
                    <span style={{fontSize:10,color:"#aaa",fontStyle:"italic"}}>thinking...</span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:4,height:16}}>
                    <div className="think-dot" style={{animationDelay:"-0.32s"}}/>
                    <div className="think-dot" style={{animationDelay:"-0.16s"}}/>
            ...
            {loading&&(
              <div style={{display:"flex",gap:12,marginBottom:20,animation:"fadeIn 0.4s both",
                paddingLeft:4}}>
                <ThinkingIndicator/>
              </div>
            )}
